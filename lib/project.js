...
const registry_manager_1 = __importDefault(require("./install/registry-manager"));
const cache_1 = __importDefault(require("./utils/cache"));
const global_config_file_1 = __importDefault(require("./config/global-config-file"));
const fetch_1 = __importDefault(require("./install/fetch"));
...
class Project {
    constructor(projectPath, options) {
        ...
        if (projectPath === api_1.JSPM_GLOBAL_PATH)
            this.checkGlobalBin();
...
class CLILogger {
    ...
    err(msg) {
        ...
        ui_1.log(this.label + msg, ui_1.LogType.err);
    }
    debug(msg) {
        ui_1.log(this.label + msg, ui_1.LogType.debug);
    }
    info(msg) {
        ui_1.log(this.label + msg, ui_1.LogType.info);
    }
    warn(msg) {
        ui_1.log(this.label + msg, ui_1.LogType.warn);
    }
    ok(msg) {
        ui_1.log(this.label + msg, ui_1.LogType.ok);
    }
    taskStart(name) {
        this.tasks.push(name);
        ui_1.log(this.label + this.tasks[this.tasks.length - 1], ui_1.LogType.status);
        if (this.tasks.length === 1)
            ui_1.startSpinner();
        // allow debug log state to expand status
        if (ui_1.logLevel === ui_1.LogType.debug && this.lastTask)
            ui_1.log(this.label + this.lastTask, ui_1.LogType.debug);
        this.lastTask = name;
