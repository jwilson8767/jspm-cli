...
const ui = __importStar(require("./utils/ui"));
const fs = require("fs");
...
const publish_1 = __importDefault(require("./install/publish"));
...
async function cliHandler(projectPaths, cmd, args) {
            ...
            case 'build':
                {
                    for (const projectPath of projectPaths) {
                        ...
                        if (options.out && projectPaths.length > 1)
                            throw new common_1.JspmUserError(`${common_1.bold('jspm build --out')} does not support execution in multiple projects.`);
                        if (options.node)
